 ...
 public:
  void AddFriend(const std::string& name, const std::string& friendName) {
    if (friendName == "qwerty") {
      // это просто для теста исключение, моделируем что в базе данных произошло
      // исключение
...
class User {
 public:
  explicit User(const std::string& name, UserDatabase* pDB)
      : name_(name), pDB_(pDB) {}
  std::string GetName() const { return name_; }
  void AddFriend(User& newFriend) {
    friends_.push_back(&newFriend);
    // ScopeGuard removeFriend = ObjScopeGuardImpl0<UserCont, void
    // (UserCont::*)()>::MakeObjGuard(friends_, &UserCont::pop_back); auto
    // removeFriend = MakeGuard([&friends = friends_]()
    //                               { friends.pop_back(); }); // это должен
    //                               быть ON_BLOCK_FAIL
...
enum class ErrorBehavior { SUCCESS, HANDLED_ERROR, UNHANDLED_ERROR };
